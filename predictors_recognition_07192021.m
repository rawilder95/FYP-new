...
for subj = 1:length(nsubj)
    for ses= 1:length(nses)
        if ~isempty(data.recalls(data.subject== nsubj(subj) & data.session== nses(ses)))
            ...
            presitemnos= data.pres_itemnos(ifr_idx,:);
            find_nan= presitemnos(isnan(recognized));
% %             Mask out items that were not tested in FR
            recitemnos(ismember(recitemnos, find_nan))= nan;
            find_notrec= presitemnos(recognized==0);
            
            recall(isnan(recitemnos))=nan;
            ifr_mask= ismember(recitemnos, find_notrec);
            sp12_num= nansum(nansum(recognized(:,12)));
            sp12_denom= nansum(nansum((recall==12)));
            sp12_prop{subj,ses}= sp12_num./sp12_denom;
            if any(any(isnan(recognized)))
                break
            end 
        end     
...
% Get rid of empty cells and transform cell to a matrix
